...
#define FIFO_OE_H()     PORT7.PODR.BIT.B1 = 1U  
#define FIFO_OE_L()     PORT7.PODR.BIT.B1 = 0U          /*拉低使FIFO输出使能*/

#define FIFO_WRST_H()   PORT3.PODR.BIT.B1 = 1U	        /*拉高允许FIFO写(数据from摄像头)指针运动 */
#define FIFO_WRST_L()   PORT3.PODR.BIT.B1 = 0U	        /*拉低使FIFO写(数据from摄像头)指针复位*/

#define FIFO_RRST_H()   PORT7.PODR.BIT.B2 = 1U	        /*拉高允许FIFO读(数据从FIFO输出)指针运动 */
#define FIFO_RRST_L()   PORT7.PODR.BIT.B2 = 0U	        /*拉低使FIFO读(数据从FIFO输出)指针复位 */

#define FIFO_RCLK_H()   PORT7.PODR.BIT.B3 = 1U	  
#define FIFO_RCLK_L()   PORT7.PODR.BIT.B3 = 0U	        /*FIFO输出数据时钟*/

#define FIFO_WE_H()     PORT3.PODR.BIT.B0 = 1U	  /*拉高使FIFO写允许*/
#define FIFO_WE_L()     PORT3.PODR.BIT.B0 = 0U


#define READ_FIFO_PIXEL(RGB565)   	do{\
	                                  RGB565=0;\
	                                  FIFO_RCLK_L();\
                                          RGB565 = (PORT4.PIDR.BYTE);\
                                          RGB565 <<= 8;\
	                                  FIFO_RCLK_H();\
                                          FIFO_RCLK_L();\
	                                  RGB565 |= PORT4.PIDR.BYTE;\
...
void color_tracking();
extern void camera_black(void);
extern void camera_red(void);
